...
- (void)touchesBegan:(NSSet<UITouch *> *)touches withEvent:(UIEvent *)event
{
    ...
    [self changeHitPointViewCenterToPoint:point];
    
    NSArray <ARHitTestResult *> *results = [self.scnView hitTest:point types:ARHitTestResultTypeFeaturePoint];
    
    ARHitTestResult *result = [results firstObject];
    
    if (result)
    {
        NSLog(@"目标距离：%f米", result.distance);
    }else
    {
        NSLog(@"未命中到目标");
    }
...
//使用已存在的平面
//NSArray <ARHitTestResult *> *results = [self.scnView hitTest:point types:ARHitTestResultTypeExistingPlane];

//使用估算的平面
//NSArray <ARHitTestResult *> *results = [self.scnView hitTest:point types:ARHitTestResultTypeEstimatedHorizontalPlane];
